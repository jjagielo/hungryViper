...
static HashTableEntry *findItem(HashTable *hashTable, unsigned int key)
{
    ...
    HashTableEntry *currNode = hashTable->buckets[index];
    while (currNode) {
        if (currNode->key == key) {
            return currNode;
        }
...
void destroyHashTable(HashTable *hashTable)
{
    ...
    for (i = 0; i < (hashTable->num_buckets); i++) {
        ...
        if (hashTable->buckets[i]) {
            HashTableEntry* currNode = hashTable->buckets[i];
...
void *insertItem(HashTable *hashTable, unsigned int key, void *value)
{
    ...
    if(!newNode) {     // Edge case in case malloc fails in createHashTableEntry
        return NULL;
    }
    ...
    if (node && node->key == key) {
        ...
        while (node && node->next) {
            if (node->next->key == key) {
                ...
                HashTableEntry* removeNode = node->next;
